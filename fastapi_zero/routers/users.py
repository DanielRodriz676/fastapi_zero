...
async def read_users(session: T_Session, filter_users: Annotated[FilterPage, Query()]):
    query = await session.scalars(select(User).limit(filter_users.limit).offset(filter_users.offset))
    users = query.all()
    return {'users': users}
...
async def read_user(id: int, session: T_Session):
    db_user = await session.scalar(select(User).where(User.id == id))
    if not db_user:
